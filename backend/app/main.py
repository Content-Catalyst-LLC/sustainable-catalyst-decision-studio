...
from app.tradeoff_matrix import (
    CRITERIA_SET_SCHEMA,
    ALTERNATIVES_SET_SCHEMA,
    TRADEOFF_MATRIX_SCHEMA,
    TRADEOFF_DIAGNOSTICS_SCHEMA,
    TradeoffMatrixRequest,
    criteria_set_template,
    alternatives_set_template,
    tradeoff_matrix_template,
    build_criteria_set,
    build_alternatives_set,
    build_tradeoff_matrix,
    attach_tradeoff_matrix,
)
from app.uncertainty_confidence import (
    UNCERTAINTY_REGISTER_SCHEMA,
    SENSITIVITY_ANALYSIS_SCHEMA,
    CONFIDENCE_ASSESSMENT_SCHEMA,
    UncertaintyConfidenceRequest,
    uncertainty_register_template,
    sensitivity_analysis_template,
    confidence_assessment_template,
    build_uncertainty_register,
    run_sensitivity_analysis,
    assess_confidence,
    attach_uncertainty_confidence,
)
from app.scenario_stress import (
    SCENARIO_SET_SCHEMA,
    SCENARIO_COMPARISON_SCHEMA,
    STRESS_TEST_SUITE_SCHEMA,
    ScenarioStressRequest,
    scenario_set_template as scenario_set_template_v250,
    scenario_comparison_template as scenario_comparison_template_v250,
    stress_test_suite_template,
    build_scenario_set,
    compare_scenarios,
    run_stress_tests,
...
from app.dependency_graph import (
    ...
    DEPENDENCY_DIAGNOSTICS_SCHEMA,
    CHANGE_IMPACT_SCHEMA,
    DependencyGraphRequest,
    graph_template as dependency_graph_template,
    diagnostics_template as dependency_diagnostics_template,
    impact_template as change_impact_template,
    build_dependency_graph,
    validate_dependency_graph,
    assess_change_impact,
    attach_dependency_graph,
)
from app.site_intelligence_context import (
    SITE_CONTEXT_BUNDLE_SCHEMA,
    SITE_SIGNAL_SNAPSHOT_SCHEMA,
    SITE_CONTEXT_RECEIPT_SCHEMA,
    SiteIntelligenceContextRequest,
    context_contracts_template,
...
from app.recommendation_review import (
    ...
    challenge_template,
    review_template as recommendation_review_template,
    build_recommendation_candidate,
    validate_recommendation_candidate,
    create_challenge,
    resolve_challenge,
    validate_challenge,
    evaluate_recommendation,
    apply_human_disposition,
    validate_review as validate_recommendation_review,
    attach_recommendation_review,
)
...
EXPENSIVE_PUBLIC_PATHS = {
    ...
    "/source-bundle/build", "/evidence-bundle/build", "/evidence-bundle/merge",
    "/decision-object/evidence", "/decision-packet/evidence-bundle",
    "/criteria/build", "/alternatives/build", "/tradeoff-matrix/build",
    "/decision-object/tradeoffs", "/decision-packet/tradeoff-matrix",
    "/uncertainty-register/build", "/sensitivity-analysis/run", "/confidence-assessment/build",
    ...
    "/decision-dependency-graph/build", "/decision-dependency-graph/validate", "/decision-dependency-graph/impact",
...
def release_manifest() -> Dict[str, Any]:
    return {
        ...
        "evidence_coverage_schema": EVIDENCE_COVERAGE_SCHEMA,
        "criteria_set_schema": CRITERIA_SET_SCHEMA,
        ...
        "uncertainty_register_schema": UNCERTAINTY_REGISTER_SCHEMA,
        "sensitivity_analysis_schema": SENSITIVITY_ANALYSIS_SCHEMA,
        "confidence_assessment_schema": CONFIDENCE_ASSESSMENT_SCHEMA,
        ...
        "site_intelligence_context_bundle_schema": SITE_CONTEXT_BUNDLE_SCHEMA,
        "site_intelligence_signal_snapshot_schema": SITE_SIGNAL_SNAPSHOT_SCHEMA,
        "site_intelligence_context_receipt_schema": SITE_CONTEXT_RECEIPT_SCHEMA,
        ...
        "compatibility": {
            ...
            "pairwise_overlap_diagnostics": True,
...
@app.get("/source-bundle/template")
def source_bundle_template_endpoint():
    return {"ok": True, "version": APP_VERSION, "source_bundle": source_bundle_template(APP_VERSION)}


@app.get("/evidence-bundle/template")
def evidence_bundle_template_endpoint():
    return {"ok": True, "version": APP_VERSION, "evidence_bundle": evidence_bundle_template(APP_VERSION)}


@app.post("/source-bundle/build")
def source_bundle_build_endpoint(req: EvidenceBundleRequest):
    bundle = build_source_bundle(req.sources, APP_VERSION)
    return {"ok": True, "version": APP_VERSION, "source_bundle": bundle}


@app.post("/evidence-bundle/build")
def evidence_bundle_build_endpoint(req: EvidenceBundleRequest):
    decision_id = str((req.decisionObject or {}).get("decision_id") or (req.packet or {}).get("decision_packet_id") or "")
    bundle = build_evidence_bundle(req.evidence, req.sources, APP_VERSION, decision_id=decision_id, source_bundle=req.sourceBundle)
    return {"ok": True, "version": APP_VERSION, "evidence_bundle": bundle}


@app.post("/evidence-bundle/merge")
def evidence_bundle_merge_endpoint(req: EvidenceBundleRequest):
    decision_id = str((req.decisionObject or {}).get("decision_id") or (req.packet or {}).get("decision_packet_id") or "")
    bundle = merge_evidence_bundles(req.bundles, APP_VERSION, decision_id=decision_id)
    return {"ok": True, "version": APP_VERSION, "evidence_bundle": bundle}


@app.post("/decision-object/evidence")
def decision_object_evidence_endpoint(req: EvidenceBundleRequest):
    ...
    decision_id = str(obj.get("decision_id") or (req.packet or {}).get("decision_packet_id") or "")
    bundle = deepcopy(req.evidenceBundle) if req.evidenceBundle.get("schema") == EVIDENCE_BUNDLE_SCHEMA else build_evidence_bundle(req.evidence, req.sources, APP_VERSION, decision_id=decision_id, source_bundle=req.sourceBundle)
    obj = attach_evidence_bundle(obj, bundle, APP_VERSION)
    obj["mapping_summary"] = decision_object_completeness(obj)
    return {"ok": True, "version": APP_VERSION, "decision_object": obj, "evidence_bundle": bundle}


@app.post("/decision-packet/evidence-bundle")
def decision_packet_evidence_bundle_endpoint(req: EvidenceBundleRequest):
    obj = deepcopy(req.decisionObject) if req.decisionObject else decision_object_from_packet(req.packet, APP_VERSION, DECISION_PACKET_SCHEMA)
    decision_id = str(obj.get("decision_id") or (req.packet or {}).get("decision_packet_id") or "")
    bundle = deepcopy(req.evidenceBundle) if req.evidenceBundle.get("schema") == EVIDENCE_BUNDLE_SCHEMA else build_evidence_bundle(req.evidence, req.sources, APP_VERSION, decision_id=decision_id, source_bundle=req.sourceBundle)
    obj = attach_evidence_bundle(obj, bundle, APP_VERSION)
    obj["mapping_summary"] = decision_object_completeness(obj)
    packet = deepcopy(req.packet) if req.packet else decision_object_to_packet(obj, APP_VERSION, DECISION_PACKET_SCHEMA)
    packet["evidence_bundle"] = deepcopy(bundle)
    packet["source_bundle"] = deepcopy(bundle.get("source_bundle", {}))
    packet["evidence_registry"] = deepcopy(bundle.get("evidence_records", []))
    packet["decision_object"] = {key: deepcopy(value) for key, value in obj.items() if key != "source_packet"}
    return {"ok": True, "version": APP_VERSION, "decision_object": obj, "evidence_bundle": bundle, "decision_packet": packet}


@app.get("/criteria/template")
def criteria_template_endpoint():
    return {"ok": True, "version": APP_VERSION, "criteria_set": criteria_set_template(APP_VERSION)}


@app.get("/alternatives/template")
def alternatives_template_endpoint():
    return {"ok": True, "version": APP_VERSION, "alternatives_set": alternatives_set_template(APP_VERSION)}


@app.get("/tradeoff-matrix/template")
def tradeoff_matrix_template_endpoint():
    return {"ok": True, "version": APP_VERSION, "tradeoff_matrix": tradeoff_matrix_template(APP_VERSION)}


@app.post("/criteria/build")
def criteria_build_endpoint(req: TradeoffMatrixRequest):
    return {"ok": True, "version": APP_VERSION, "criteria_set": build_criteria_set(req.criteria, APP_VERSION)}


@app.post("/alternatives/build")
def alternatives_build_endpoint(req: TradeoffMatrixRequest):
    return {"ok": True, "version": APP_VERSION, "alternatives_set": build_alternatives_set(req.alternatives, APP_VERSION)}


@app.post("/tradeoff-matrix/build")
def tradeoff_matrix_build_endpoint(req: TradeoffMatrixRequest):
    decision_id = str((req.decisionObject or {}).get("decision_id") or (req.packet or {}).get("decision_packet_id") or "")
    matrix = build_tradeoff_matrix(req.criteria, req.alternatives, req.evaluations, APP_VERSION, decision_id=decision_id, criteria_set=req.criteriaSet, alternatives_set=req.alternativesSet)
    return {"ok": True, "version": APP_VERSION, "tradeoff_matrix": matrix}


@app.post("/decision-object/tradeoffs")
def decision_object_tradeoffs_endpoint(req: TradeoffMatrixRequest):
    obj = deepcopy(req.decisionObject) if req.decisionObject else decision_object_from_packet(req.packet, APP_VERSION, DECISION_PACKET_SCHEMA)
    decision_id = str(obj.get("decision_id") or (req.packet or {}).get("decision_packet_id") or "")
    matrix = deepcopy(req.tradeoffMatrix) if req.tradeoffMatrix.get("schema") == TRADEOFF_MATRIX_SCHEMA else build_tradeoff_matrix(req.criteria, req.alternatives, req.evaluations, APP_VERSION, decision_id=decision_id, criteria_set=req.criteriaSet, alternatives_set=req.alternativesSet)
    obj = attach_tradeoff_matrix(obj, matrix, APP_VERSION)
    obj["mapping_summary"] = decision_object_completeness(obj)
    return {"ok": True, "version": APP_VERSION, "decision_object": obj, "tradeoff_matrix": matrix}


@app.post("/decision-packet/tradeoff-matrix")
def decision_packet_tradeoff_matrix_endpoint(req: TradeoffMatrixRequest):
    obj = deepcopy(req.decisionObject) if req.decisionObject else decision_object_from_packet(req.packet, APP_VERSION, DECISION_PACKET_SCHEMA)
    decision_id = str(obj.get("decision_id") or (req.packet or {}).get("decision_packet_id") or "")
    matrix = deepcopy(req.tradeoffMatrix) if req.tradeoffMatrix.get("schema") == TRADEOFF_MATRIX_SCHEMA else build_tradeoff_matrix(req.criteria, req.alternatives, req.evaluations, APP_VERSION, decision_id=decision_id, criteria_set=req.criteriaSet, alternatives_set=req.alternativesSet)
    obj = attach_tradeoff_matrix(obj, matrix, APP_VERSION)
    obj["mapping_summary"] = decision_object_completeness(obj)
    packet = deepcopy(req.packet) if req.packet else decision_object_to_packet(obj, APP_VERSION, DECISION_PACKET_SCHEMA)
    packet["criteria_set"] = deepcopy(matrix.get("criteria_set", {}))
    packet["alternatives_set"] = deepcopy(matrix.get("alternatives_set", {}))
    packet["tradeoff_matrix"] = deepcopy(matrix)
    packet["criteria_registry"] = deepcopy(matrix.get("criteria_set", {}).get("criteria", []))
    packet["alternatives"] = deepcopy(matrix.get("alternatives_set", {}).get("alternatives", []))
    packet["tradeoffs"] = deepcopy(matrix.get("evaluations", []))
    packet["decision_object"] = {key: deepcopy(value) for key, value in obj.items() if key != "source_packet"}
    return {"ok": True, "version": APP_VERSION, "decision_object": obj, "tradeoff_matrix": matrix, "decision_packet": packet}


@app.get("/uncertainty-register/template")
def uncertainty_register_template_endpoint():
    return {"ok": True, "version": APP_VERSION, "uncertainty_register": uncertainty_register_template(APP_VERSION)}


@app.get("/sensitivity-analysis/template")
def sensitivity_analysis_template_endpoint():
    return {"ok": True, "version": APP_VERSION, "sensitivity_analysis": sensitivity_analysis_template(APP_VERSION)}


@app.get("/confidence-assessment/template")
def confidence_assessment_template_endpoint():
    return {"ok": True, "version": APP_VERSION, "confidence_assessment": confidence_assessment_template(APP_VERSION)}


def _resolve_uncertainty_v240(req: UncertaintyConfidenceRequest):
    obj = deepcopy(req.decisionObject) if req.decisionObject else decision_object_from_packet(req.packet, APP_VERSION, DECISION_PACKET_SCHEMA)
    matrix = deepcopy(req.tradeoffMatrix)
    if matrix.get("schema") != TRADEOFF_MATRIX_SCHEMA:
        matrices = obj.get("tradeoff_matrices", []) if isinstance(obj.get("tradeoff_matrices"), list) else []
        matrix = deepcopy(matrices[-1]) if matrices else {}
    decision_id = str(obj.get("decision_id") or (req.packet or {}).get("decision_packet_id") or matrix.get("decision_id") or "")
    register = deepcopy(req.uncertaintyRegister) if req.uncertaintyRegister.get("schema") == UNCERTAINTY_REGISTER_SCHEMA else build_uncertainty_register(req.uncertainties, matrix, APP_VERSION, decision_id=decision_id)
    sensitivity = deepcopy(req.sensitivityAnalysis) if req.sensitivityAnalysis.get("schema") == SENSITIVITY_ANALYSIS_SCHEMA else run_sensitivity_analysis(matrix, register, APP_VERSION, decision_id=decision_id, config=req.sensitivityConfig)
    confidence = deepcopy(req.confidenceAssessment) if req.confidenceAssessment.get("schema") == CONFIDENCE_ASSESSMENT_SCHEMA else assess_confidence(matrix, register, sensitivity, APP_VERSION, decision_id=decision_id)
    return obj, matrix, register, sensitivity, confidence


@app.post("/uncertainty-register/build")
def uncertainty_register_build_endpoint(req: UncertaintyConfidenceRequest):
    obj, matrix, register, sensitivity, confidence = _resolve_uncertainty_v240(req)
    return {"ok": True, "version": APP_VERSION, "uncertainty_register": register, "tradeoff_matrix": matrix}


@app.post("/sensitivity-analysis/run")
def sensitivity_analysis_run_endpoint(req: UncertaintyConfidenceRequest):
    obj, matrix, register, sensitivity, confidence = _resolve_uncertainty_v240(req)
    return {"ok": True, "version": APP_VERSION, "uncertainty_register": register, "sensitivity_analysis": sensitivity, "tradeoff_matrix": matrix}


@app.post("/confidence-assessment/build")
def confidence_assessment_build_endpoint(req: UncertaintyConfidenceRequest):
    obj, matrix, register, sensitivity, confidence = _resolve_uncertainty_v240(req)
    return {"ok": True, "version": APP_VERSION, "uncertainty_register": register, "sensitivity_analysis": sensitivity, "confidence_assessment": confidence, "tradeoff_matrix": matrix}


@app.post("/decision-object/uncertainty-confidence")
def decision_object_uncertainty_confidence_endpoint(req: UncertaintyConfidenceRequest):
    obj, matrix, register, sensitivity, confidence = _resolve_uncertainty_v240(req)
    obj = attach_uncertainty_confidence(obj, register, sensitivity, confidence, APP_VERSION)
    obj["mapping_summary"] = decision_object_completeness(obj)
    return {"ok": True, "version": APP_VERSION, "decision_object": obj, "tradeoff_matrix": matrix, "uncertainty_register": register, "sensitivity_analysis": sensitivity, "confidence_assessment": confidence}


@app.post("/decision-packet/uncertainty-confidence")
def decision_packet_uncertainty_confidence_endpoint(req: UncertaintyConfidenceRequest):
    obj, matrix, register, sensitivity, confidence = _resolve_uncertainty_v240(req)
    obj = attach_uncertainty_confidence(obj, register, sensitivity, confidence, APP_VERSION)
    obj["mapping_summary"] = decision_object_completeness(obj)
    packet = deepcopy(req.packet) if req.packet else decision_object_to_packet(obj, APP_VERSION, DECISION_PACKET_SCHEMA)
    packet["uncertainty_register"] = deepcopy(register)
    packet["sensitivity_analysis"] = deepcopy(sensitivity)
    packet["confidence_assessment"] = deepcopy(confidence)
    packet["uncertainties"] = deepcopy(register.get("uncertainties", []))
    packet["confidence"] = deepcopy(obj.get("confidence", {}))
    packet["decision_object"] = {key: deepcopy(value) for key, value in obj.items() if key != "source_packet"}
    return {"ok": True, "version": APP_VERSION, "decision_object": obj, "decision_packet": packet, "tradeoff_matrix": matrix, "uncertainty_register": register, "sensitivity_analysis": sensitivity, "confidence_assessment": confidence}


@app.get("/scenario-set/template")
def scenario_set_template_v250_endpoint():
    return {"ok": True, "version": APP_VERSION, "scenario_set": scenario_set_template_v250(APP_VERSION)}


@app.get("/scenario-analysis/template")
def scenario_comparison_template_v250_endpoint():
    return {"ok": True, "version": APP_VERSION, "scenario_comparison": scenario_comparison_template_v250(APP_VERSION)}


@app.get("/stress-test-suite/template")
def stress_test_suite_template_endpoint():
    return {"ok": True, "version": APP_VERSION, "stress_test_suite": stress_test_suite_template(APP_VERSION)}


def _resolve_scenario_v250(req: ScenarioStressRequest):
    obj = deepcopy(req.decisionObject) if req.decisionObject else decision_object_from_packet(req.packet, APP_VERSION, DECISION_PACKET_SCHEMA)
    matrix = deepcopy(req.tradeoffMatrix)
    if matrix.get("schema") != TRADEOFF_MATRIX_SCHEMA:
        matrices = obj.get("tradeoff_matrices", []) if isinstance(obj.get("tradeoff_matrices"), list) else []
        matrix = deepcopy(matrices[-1]) if matrices else {}
    decision_id = str(obj.get("decision_id") or (req.packet or {}).get("decision_packet_id") or matrix.get("decision_id") or "")
    scenario_set = deepcopy(req.scenarioSet) if req.scenarioSet.get("schema") == SCENARIO_SET_SCHEMA else build_scenario_set(req.scenarios, matrix, APP_VERSION, decision_id=decision_id)
    comparison = deepcopy(req.scenarioComparison) if req.scenarioComparison.get("schema") == SCENARIO_COMPARISON_SCHEMA else compare_scenarios(matrix, scenario_set, APP_VERSION, decision_id=decision_id)
    confidence = deepcopy(req.confidenceAssessment) if req.confidenceAssessment.get("schema") == CONFIDENCE_ASSESSMENT_SCHEMA else {}
    stress = deepcopy(req.stressTestSuite) if req.stressTestSuite.get("schema") == STRESS_TEST_SUITE_SCHEMA else run_stress_tests(comparison, scenario_set, confidence, APP_VERSION, decision_id=decision_id, config=req.stressConfig)
    return obj, matrix, scenario_set, comparison, stress


@app.post("/scenario-set/build")
def scenario_set_build_v250_endpoint(req: ScenarioStressRequest):
    obj, matrix, scenario_set, comparison, stress = _resolve_scenario_v250(req)
    return {"ok": True, "version": APP_VERSION, "scenario_set": scenario_set, "tradeoff_matrix": matrix}


@app.post("/scenario-analysis/compare")
def scenario_compare_v250_endpoint(req: ScenarioStressRequest):
    obj, matrix, scenario_set, comparison, stress = _resolve_scenario_v250(req)
    return {"ok": True, "version": APP_VERSION, "scenario_set": scenario_set, "scenario_comparison": comparison, "tradeoff_matrix": matrix}


@app.post("/stress-test-suite/run")
def stress_test_suite_run_v250_endpoint(req: ScenarioStressRequest):
    obj, matrix, scenario_set, comparison, stress = _resolve_scenario_v250(req)
    return {"ok": True, "version": APP_VERSION, "scenario_set": scenario_set, "scenario_comparison": comparison, "stress_test_suite": stress, "tradeoff_matrix": matrix}


@app.post("/decision-object/scenario-stress")
def decision_object_scenario_stress_v250_endpoint(req: ScenarioStressRequest):
    obj, matrix, scenario_set, comparison, stress = _resolve_scenario_v250(req)
    obj = attach_scenario_stress(obj, scenario_set, comparison, stress, APP_VERSION)
    obj["mapping_summary"] = decision_object_completeness(obj)
    return {"ok": True, "version": APP_VERSION, "decision_object": obj, "scenario_set": scenario_set, "scenario_comparison": comparison, "stress_test_suite": stress, "tradeoff_matrix": matrix}


@app.post("/decision-packet/scenario-stress")
def decision_packet_scenario_stress_v250_endpoint(req: ScenarioStressRequest):
    obj, matrix, scenario_set, comparison, stress = _resolve_scenario_v250(req)
    obj = attach_scenario_stress(obj, scenario_set, comparison, stress, APP_VERSION)
    obj["mapping_summary"] = decision_object_completeness(obj)
    packet = deepcopy(req.packet) if req.packet else decision_object_to_packet(obj, APP_VERSION, DECISION_PACKET_SCHEMA)
    packet["scenario_set"] = deepcopy(scenario_set)
    packet["scenario_stress_comparison"] = deepcopy(comparison)
    packet["stress_test_suite"] = deepcopy(stress)
...
@app.get("/site-intelligence-context/contracts")
def site_intelligence_context_contracts_endpoint():
    return {"ok": True, "version": APP_VERSION, "contracts": context_contracts_template(APP_VERSION)}


@app.get("/site-intelligence-context/template")
def site_intelligence_context_template_endpoint():
    return {"ok": True, "version": APP_VERSION, "context_bundle": context_bundle_template(APP_VERSION)}


def _resolve_site_intelligence_context(req: SiteIntelligenceContextRequest):
    packet = deepcopy(req.packet or {})
    obj = deepcopy(req.decisionObject or {})
    if not obj and packet:
        obj = decision_object_from_packet(packet, APP_VERSION, DECISION_PACKET_SCHEMA)
    decision_id = str(obj.get("decision_id") or packet.get("decision_packet_id") or "")
    bundle = deepcopy(req.contextBundle or {})
    if not bundle:
        bundle = build_context_bundle(
            ...
            scenario_refs=req.scenarioRefs,
            review_state=req.reviewState,
            provenance=req.provenance,
        )
    validation = validate_context_bundle(bundle, app_version=APP_VERSION)
    receipt = context_receipt(bundle, validation, app_version=APP_VERSION)
    return packet, obj, bundle, validation, receipt


@app.post("/site-intelligence-context/build")
def site_intelligence_context_build_endpoint(req: SiteIntelligenceContextRequest):
    packet, obj, bundle, validation, receipt = _resolve_site_intelligence_context(req)
    return {"ok": validation["valid"], "version": APP_VERSION, "context_bundle": bundle, "validation": validation, "receipt": receipt}


@app.post("/site-intelligence-context/validate")
def site_intelligence_context_validate_endpoint(req: SiteIntelligenceContextRequest):
    packet, obj, bundle, validation, receipt = _resolve_site_intelligence_context(req)
    return {"ok": validation["valid"], "version": APP_VERSION, "context_bundle": bundle, "validation": validation, "receipt": receipt}


@app.post("/decision-object/site-intelligence-context")
def decision_object_site_intelligence_context_endpoint(req: SiteIntelligenceContextRequest):
    packet, obj, bundle, validation, receipt = _resolve_site_intelligence_context(req)
    if validation["valid"]:
        obj = attach_site_context(obj, bundle, receipt, app_version=APP_VERSION)
        obj["mapping_summary"] = decision_object_completeness(obj)
    return {"ok": validation["valid"], "version": APP_VERSION, "context_bundle": bundle, "validation": validation, "receipt": receipt, "decision_object": obj}


@app.post("/decision-packet/site-intelligence-context")
def decision_packet_site_intelligence_context_endpoint(req: SiteIntelligenceContextRequest):
    packet, obj, bundle, validation, receipt = _resolve_site_intelligence_context(req)
    if validation["valid"]:
        obj = attach_site_context(obj, bundle, receipt, app_version=APP_VERSION)
        obj["mapping_summary"] = decision_object_completeness(obj)
        packet = deepcopy(packet) if packet else decision_object_to_packet(obj, APP_VERSION, DECISION_PACKET_SCHEMA)
        packet.setdefault("site_intelligence_context_bundles", []).append(deepcopy(bundle))
        packet.setdefault("site_intelligence_context_receipts", []).append(deepcopy(receipt))
        packet.setdefault("live_evidence", []).extend(deepcopy(bundle.get("snapshots") or []))
        packet["decision_object"] = {key: deepcopy(value) for key, value in obj.items() if key != "source_packet"}
    return {"ok": validation["valid"], "version": APP_VERSION, "context_bundle": bundle, "validation": validation, "receipt": receipt, "decision_object": obj, "decision_packet": packet}


@app.get("/decision-dependency-graph/template")
def decision_dependency_graph_template_endpoint():
    return {
        "ok": True,
        "version": APP_VERSION,
        "dependency_graph": dependency_graph_template(APP_VERSION),
        "dependency_diagnostics": dependency_diagnostics_template(APP_VERSION),
        "change_impact": change_impact_template(APP_VERSION),
    }


def _resolve_dependency_graph(req: DependencyGraphRequest):
    packet = deepcopy(req.packet or {})
    obj = deepcopy(req.decisionObject or {})
    if not obj and packet:
        obj = decision_object_from_packet(packet, APP_VERSION, DECISION_PACKET_SCHEMA)
    graph = deepcopy(req.graph or {})
    if not graph:
        graph = build_dependency_graph(
            obj,
            app_version=APP_VERSION,
            explicit_edges=req.explicitEdges,
            include_structural_edges=req.includeStructuralEdges,
        )
    validation = validate_dependency_graph(graph, app_version=APP_VERSION)
    return packet, obj, graph, validation


@app.post("/decision-dependency-graph/build")
def decision_dependency_graph_build_endpoint(req: DependencyGraphRequest):
    packet, obj, graph, validation = _resolve_dependency_graph(req)
    return {"ok": validation["valid"], "version": APP_VERSION, "dependency_graph": graph, "validation": validation}


@app.post("/decision-dependency-graph/validate")
def decision_dependency_graph_validate_endpoint(req: DependencyGraphRequest):
    packet, obj, graph, validation = _resolve_dependency_graph(req)
    return {"ok": validation["valid"], "version": APP_VERSION, "dependency_graph": graph, "validation": validation}


@app.post("/decision-dependency-graph/impact")
def decision_dependency_graph_impact_endpoint(req: DependencyGraphRequest):
    packet, obj, graph, validation = _resolve_dependency_graph(req)
    impact = assess_change_impact(graph, req.changedNodeIds, app_version=APP_VERSION) if validation["valid"] else change_impact_template(APP_VERSION)
    return {"ok": validation["valid"], "version": APP_VERSION, "dependency_graph": graph, "validation": validation, "change_impact": impact}


@app.post("/decision-object/dependency-graph")
def decision_object_dependency_graph_endpoint(req: DependencyGraphRequest):
    packet, obj, graph, validation = _resolve_dependency_graph(req)
    if validation["valid"]:
        obj = attach_dependency_graph(obj, graph, validation, app_version=APP_VERSION)
        obj["mapping_summary"] = decision_object_completeness(obj)
    return {"ok": validation["valid"], "version": APP_VERSION, "dependency_graph": graph, "validation": validation, "decision_object": obj}


@app.post("/decision-packet/dependency-graph")
def decision_packet_dependency_graph_endpoint(req: DependencyGraphRequest):
    packet, obj, graph, validation = _resolve_dependency_graph(req)
    if validation["valid"]:
        obj = attach_dependency_graph(obj, graph, validation, app_version=APP_VERSION)
        obj["mapping_summary"] = decision_object_completeness(obj)
        packet = deepcopy(packet) if packet else decision_object_to_packet(obj, APP_VERSION, DECISION_PACKET_SCHEMA)
        packet.setdefault("decision_dependency_graphs", []).append(deepcopy(graph))
        packet.setdefault("dependency_diagnostics", []).append({"graph_id": graph.get("graph_id", ""), **deepcopy(validation.get("diagnostics") or {})})
        packet["decision_object"] = {key: deepcopy(value) for key, value in obj.items() if key != "source_packet"}
    return {"ok": validation["valid"], "version": APP_VERSION, "dependency_graph": graph, "validation": validation, "decision_object": obj, "decision_packet": packet}


@app.get("/recommendation-review/template")
def recommendation_review_template_endpoint():
    return {
        "ok": True,
        "version": APP_VERSION,
        "recommendation_candidate": recommendation_template(APP_VERSION),
        "recommendation_challenge": challenge_template(APP_VERSION),
        "recommendation_review": recommendation_review_template(APP_VERSION),
    }


def _resolve_recommendation_review(req: RecommendationReviewRequest):
    packet = deepcopy(req.packet or {})
    obj = deepcopy(req.decisionObject or {})
    if not obj and packet:
        obj = decision_object_from_packet(packet, APP_VERSION, DECISION_PACKET_SCHEMA)
    graph = deepcopy(req.graph or {})
    if not graph:
        graphs = obj.get("decision_dependency_graphs") or []
        if graphs and isinstance(graphs[-1], dict):
            graph = deepcopy(graphs[-1])
    candidate = deepcopy(req.candidate or {})
    if not candidate and req.selectedAlternativeId:
        candidate = build_recommendation_candidate(
            obj, graph, app_version=APP_VERSION, selected_alternative_id=req.selectedAlternativeId,
            rationale=req.rationale, support_node_ids=req.supportNodeIds, counter_node_ids=req.counterNodeIds,
            conditions=req.conditions, required_reviews=req.requiredReviews,
        )
    candidate_validation = validate_recommendation_candidate(candidate, app_version=APP_VERSION) if candidate else {"valid": False, "errors": ["candidate is required"]}
    challenges = deepcopy(req.challenges or [])
    review = deepcopy(req.review or {})
    if candidate and not review:
        review = evaluate_recommendation(candidate, challenges, graph, app_version=APP_VERSION)
    return packet, obj, graph, candidate, candidate_validation, challenges, review


@app.post("/recommendation-review/candidate")
def recommendation_review_candidate_endpoint(req: RecommendationReviewRequest):
    try:
        packet, obj, graph, candidate, validation, challenges, review = _resolve_recommendation_review(req)
        return {"ok": validation["valid"], "version": APP_VERSION, "candidate": candidate, "validation": validation, "review": review}
    except ValueError as exc:
        return JSONResponse(status_code=400, content={"ok": False, "version": APP_VERSION, "error": str(exc)})


@app.post("/recommendation-review/challenge")
def recommendation_review_challenge_endpoint(req: RecommendationReviewRequest):
    try:
        packet, obj, graph, candidate, validation, challenges, review = _resolve_recommendation_review(req)
        if not validation.get("valid"):
            return JSONResponse(status_code=400, content={"ok": False, "version": APP_VERSION, "error": "valid candidate required", "validation": validation})
        if req.challengeAction == "create":
            challenge = create_challenge(candidate, app_version=APP_VERSION, challenge_type=req.challengeType, statement=req.statement, target_node_ids=req.targetNodeIds, evidence_refs=req.evidenceRefs, actor=req.actor)
            challenges.append(challenge)
        else:
            challenge = next((deepcopy(x) for x in challenges if isinstance(x, dict) and x.get("challenge_id") == req.challengeId), {})
            if not challenge:
                raise ValueError("challengeId does not match a supplied challenge")
            updated = resolve_challenge(challenge, app_version=APP_VERSION, actor=req.actor, resolution=req.challengeResolution, status=req.challengeStatus)
            challenges = [updated if isinstance(x, dict) and x.get("challenge_id") == req.challengeId else x for x in challenges]
            challenge = updated
        challenge_validation = validate_challenge(challenge, app_version=APP_VERSION)
        review = evaluate_recommendation(candidate, challenges, graph, app_version=APP_VERSION)
        return {"ok": challenge_validation["valid"], "version": APP_VERSION, "candidate": candidate, "challenge": challenge, "challenge_validation": challenge_validation, "challenges": challenges, "review": review}
    except ValueError as exc:
        return JSONResponse(status_code=400, content={"ok": False, "version": APP_VERSION, "error": str(exc)})


@app.post("/recommendation-review/evaluate")
def recommendation_review_evaluate_endpoint(req: RecommendationReviewRequest):
    packet, obj, graph, candidate, validation, challenges, review = _resolve_recommendation_review(req)
    if not validation.get("valid"):
        return JSONResponse(status_code=400, content={"ok": False, "version": APP_VERSION, "error": "valid candidate required", "validation": validation})
    review = evaluate_recommendation(candidate, challenges, graph, app_version=APP_VERSION)
    return {"ok": True, "version": APP_VERSION, "candidate": candidate, "challenges": challenges, "review": review, "validation": validate_recommendation_review(review, app_version=APP_VERSION)}


@app.post("/recommendation-review/disposition")
def recommendation_review_disposition_endpoint(req: RecommendationReviewRequest):
    try:
        packet, obj, graph, candidate, validation, challenges, review = _resolve_recommendation_review(req)
        if not validation.get("valid"):
            return JSONResponse(status_code=400, content={"ok": False, "version": APP_VERSION, "error": "valid candidate required", "validation": validation})
        review = evaluate_recommendation(candidate, challenges, graph, app_version=APP_VERSION) if not review else review
        review = apply_human_disposition(review, challenges, app_version=APP_VERSION, disposition=req.disposition, actor=req.actor, rationale=req.dispositionRationale, override_open_challenges=req.overrideOpenChallenges)
        return {"ok": True, "version": APP_VERSION, "candidate": candidate, "challenges": challenges, "review": review, "validation": validate_recommendation_review(review, app_version=APP_VERSION)}
    except ValueError as exc:
        return JSONResponse(status_code=400, content={"ok": False, "version": APP_VERSION, "error": str(exc)})


def _attach_recommendation_payload(req: RecommendationReviewRequest):
    packet, obj, graph, candidate, validation, challenges, review = _resolve_recommendation_review(req)
    if not validation.get("valid"):
        return packet, obj, candidate, challenges, review, validation
    if req.disposition:
        review = apply_human_disposition(review, challenges, app_version=APP_VERSION, disposition=req.disposition, actor=req.actor, rationale=req.dispositionRationale, override_open_challenges=req.overrideOpenChallenges)
    review_validation = validate_recommendation_review(review, app_version=APP_VERSION)
    if review_validation["valid"]:
        obj = attach_recommendation_review(obj, candidate, challenges, review, app_version=APP_VERSION)
        obj["mapping_summary"] = decision_object_completeness(obj)
    return packet, obj, candidate, challenges, review, review_validation


@app.post("/decision-object/recommendation-review")
def decision_object_recommendation_review_endpoint(req: RecommendationReviewRequest):
    try:
        packet, obj, candidate, challenges, review, validation = _attach_recommendation_payload(req)
        return {"ok": validation["valid"], "version": APP_VERSION, "candidate": candidate, "challenges": challenges, "review": review, "validation": validation, "decision_object": obj}
    except ValueError as exc:
        return JSONResponse(status_code=400, content={"ok": False, "version": APP_VERSION, "error": str(exc)})


@app.post("/decision-packet/recommendation-review")
def decision_packet_recommendation_review_endpoint(req: RecommendationReviewRequest):
    try:
        packet, obj, candidate, challenges, review, validation = _attach_recommendation_payload(req)
        if validation["valid"]:
            packet = deepcopy(packet) if packet else decision_object_to_packet(obj, APP_VERSION, DECISION_PACKET_SCHEMA)
            packet.setdefault("recommendation_candidates", []).append(deepcopy(candidate))
            packet.setdefault("recommendation_challenges", []).extend(deepcopy(challenges))
            packet.setdefault("recommendation_reviews", []).append(deepcopy(review))
            packet["decision_object"] = {key: deepcopy(value) for key, value in obj.items() if key != "source_packet"}
        return {"ok": validation["valid"], "version": APP_VERSION, "candidate": candidate, "challenges": challenges, "review": review, "validation": validation, "decision_object": obj, "decision_packet": packet}
    except ValueError as exc:
        return JSONResponse(status_code=400, content={"ok": False, "version": APP_VERSION, "error": str(exc)})
...
# Introduced in Decision Studio v2.3.0 and intentionally preserved in v2.4.0.
