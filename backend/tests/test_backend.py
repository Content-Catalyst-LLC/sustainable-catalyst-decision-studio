...
def test_decision_packet_template():
    ...
    data = r.json()
    assert data['ok'] is True
    assert data['decision_packet']['packet_version'] == '2.4.0'
...
def test_brief_readiness_default():
    ...
    assert readiness['readiness_version'] == '2.4.0'
...
def test_workbench_handoff_default():
    ...
    assert handoff['handoff_version'] == '2.4.0'
...
def test_export_center_template():
    ...
    data = r.json()
    assert data['ok'] is True
    assert data['export_center']['export_center_version'] == '2.4.0'
...
def test_decision_packet_save_template():
    ...
    assert saved['packet_version'] == '2.4.0'
...
def test_public_landing_template():
    ...
    assert data['ok'] is True
    assert data['version'] == '2.4.0'
    assert data['landing']['page_version'] == '2.4.0'
...
def test_public_demo_template():
    ...
    data = r.json()
    assert data['ok'] is True
    assert data['demo']['demo_version'] == '2.4.0'
...
def test_release_manifest_identity():
    ...
    data = response.json()
    assert data['version'] == '2.4.0'
    assert data['release']['build_fingerprint'] == 'scds-v2.4.0-uncertainty-sensitivity-confidence'
...
def test_health_reports_cold_start_and_limits():
    ...
    assert response.headers['x-scds-version'] == '2.4.0'
...
def test_platform_handoff_template_has_new_packet_sections():
    ...
    assert r.status_code == 200
    packet = r.json()['decision_packet']
    assert packet['packet_version'] == '2.4.0'
...
def test_collaboration_contact_and_engagement_handoff():
    ...
    assert handoff['source_version'] == '2.4.0'
...
def test_collaboration_is_saved_and_exported_with_packet_schema_1_5():
    ...
    assert saved['decision_packet']['packet_version'] == '2.4.0'
...
def test_release_manifest_declares_v2_connected_platform():
    release = client.get('/release').json()['release']
    assert release['release'] == '2.4.0'
    assert release['release_name'] == 'Uncertainty, Sensitivity & Confidence'
