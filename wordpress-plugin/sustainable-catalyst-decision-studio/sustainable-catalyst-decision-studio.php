 ...
 * Description: Public API, secure embeds, signed institutional archives, Platform Core gateway, bulk packet exchange, outcomes monitoring, publications, Decision Packs, collaboration, scenarios, governance, and typed platform evidence.
 * Version: 1.15.0
...
class Sustainable_Catalyst_Decision_Studio {
    ...
    const BUILD_FINGERPRINT = 'scds-v1.15.0-public-api-embeds-institutional-integration';
    const SOURCE_COMMIT = 'release-v1.15.0';
    ...
    public function render_decision_studio_shortcode($atts = []) {
        ...
        if (!in_array($mode, ['full', 'landing', 'demo', 'workflow', 'readiness', 'governance', 'room', 'packs', 'publication', 'outcomes', 'integration', 'project-intake', 'scorecard', 'risk', 'scenario', 'handoff', 'packets', 'export', 'report', 'drawer', 'compact'], true)) {
            $mode = 'full';
        }
        ...
        $start_tab = $mode === 'workflow' ? 'workflow' : ($mode === 'readiness' ? 'readiness' : ($mode === 'governance' ? 'governance' : ($mode === 'room' ? 'room' : ($mode === 'packs' ? 'packs' : ($mode === 'publication' ? 'publication' : ($mode === 'outcomes' ? 'outcomes' : ($mode === 'integration' ? 'integration' : ($mode === 'project-intake' ? 'intake' : (in_array($mode, ['scorecard', 'risk', 'scenario', 'handoff', 'packets', 'export', 'report'], true) ? $mode : 'intake')))))))));
        ...
        wp_localize_script('scds-decision-studio', 'SCDSDecisionStudio', [
            ...
            'storageKey' => 'scds_saved_decision_packets_v1_15_0',
    ...
    private function public_landing_template() {
        return [
            ...
            'features' => [
                ...
                ['title'=>'Decision Packet workspace','description'=>'Use scds-decision-packet/1.8 to hold platform handoffs, evidence, assumptions, scenarios, experiments, calculations, entities, provenance, and audit records.'],
...
SCDS_OPENAI_MODEL=&lt;your-model&gt;</pre>';
    ...
    public function rest_packet_save_template(WP_REST_Request $request) {
        ...
        $publication=isset($payload['publicationStudio'])&&is_array($payload['publicationStudio'])?$payload['publicationStudio']:($packet['publication_studio']??[]);$packet['publication_studio']=$publication; $outcomes=isset($payload['outcomeMonitoring'])&&is_array($payload['outcomeMonitoring'])?$payload['outcomeMonitoring']:($packet['outcome_monitoring']??$this->outcome_monitoring_template()); $packet['outcome_monitoring']=$outcomes; $integration=isset($payload['institutionalIntegration'])&&is_array($payload['institutionalIntegration'])?$payload['institutionalIntegration']:($packet['institutional_integration']??$this->public_integration_template()); $packet['institutional_integration']=$integration;
        $saved = ['packet_version'=>self::VERSION,'decision_packet_id'=>$packet['decision_packet_id'] ?? ('SCDS-' . strtoupper(substr(preg_replace('/[^A-Za-z0-9]/','',$title),0,10)) . '-DRAFT'),'title'=>$title,'project_name'=>$title,'decision_question'=>$packet['project']['decision_question'] ?? ($inputs['decisionQuestion'] ?? ''),'status'=>$saved_status,'inputs'=>$inputs,'results'=>$results,'decision_packet'=>$packet,'audit'=>$audit,'readiness'=>$readiness,'scenario_comparison'=>$scenario,'scenario_studio'=>$scenario_studio,'workbench_handoff'=>$handoff,'integrated_brief'=>$brief,'governance'=>$packet['governance_center'] ?? $this->governance_template(),'collaboration'=>$collaboration,'decision_pack'=>($payload['decisionPack'] ?? ($packet['institutional_decision_pack'] ?? [])),'publication_studio'=>$publication,'outcome_monitoring'=>$outcomes,'institutional_integration'=>$integration,'notes'=>sanitize_textarea_field($payload['notes'] ?? ''),'warnings'=>['Saved packet is a working record, not approval or professional signoff.']];
    ...
    public function rest_export_center_bundle(WP_REST_Request $request) {
        ...
        $briefData = isset($payload['integratedBrief']) && is_array($payload['integratedBrief']) ? $payload['integratedBrief'] : $this->generate_integrated_brief($inputs,$results,$packet,$audit);
        $brief = isset($briefData['brief']) ? $briefData['brief'] : $briefData;
        $governance = isset($payload['governance']) && is_array($payload['governance']) ? $payload['governance'] : ($packet['governance_center'] ?? $this->governance_template()); $packet['governance_center']=$governance; $collaboration=isset($payload['collaboration'])&&is_array($payload['collaboration'])?$payload['collaboration']:($packet['collaboration_room']??$this->collaboration_room_template()); $packet['collaboration_room']=$collaboration; $decision_pack=isset($payload['decisionPack'])&&is_array($payload['decisionPack'])?$payload['decisionPack']:($packet['institutional_decision_pack']??[]); $packet['institutional_decision_pack']=$decision_pack; $publication=isset($payload['publicationStudio'])&&is_array($payload['publicationStudio'])?$payload['publicationStudio']:($packet['publication_studio']??[]); $packet['publication_studio']=$publication; $outcomes=isset($payload['outcomeMonitoring'])&&is_array($payload['outcomeMonitoring'])?$payload['outcomeMonitoring']:($packet['outcome_monitoring']??$this->outcome_monitoring_template()); $packet['outcome_monitoring']=$outcomes; $integration=isset($payload['institutionalIntegration'])&&is_array($payload['institutionalIntegration'])?$payload['institutionalIntegration']:($packet['institutional_integration']??$this->public_integration_template()); $packet['institutional_integration']=$integration;
    ...
    private function decision_packet_template() {
        return [
            'packet_version'=>self::VERSION,'decision_packet_schema'=>'scds-decision-packet/1.8','decision_pack_schema'=>self::DECISION_PACK_SCHEMA,'decision_pack_application_schema'=>self::DECISION_PACK_APPLICATION_SCHEMA,
            'publication_studio_schema'=>self::PUBLICATION_STUDIO_SCHEMA,
            'outcome_monitoring_schema'=>self::OUTCOME_MONITORING_SCHEMA,
            'reassessment_event_schema'=>self::REASSESSMENT_EVENT_SCHEMA,
            'decision_registry_schema'=>self::DECISION_REGISTRY_SCHEMA,
