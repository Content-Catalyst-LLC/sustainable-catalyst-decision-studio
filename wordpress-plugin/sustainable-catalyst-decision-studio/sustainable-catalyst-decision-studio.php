 ...
 * Description: Governed outcomes, monitoring, and reassessment studio with commitments, targets, observations, milestones, risks, assumption invalidation, Decision Registry records, publications, Decision Packs, collaboration, scenarios, governance, and typed platform evidence.
 * Version: 1.14.0
...
class Sustainable_Catalyst_Decision_Studio {
    ...
    const BUILD_FINGERPRINT = 'scds-v1.14.0-outcomes-monitoring-reassessment';
    const SOURCE_COMMIT = 'release-v1.14.0';
    ...
    public function render_decision_studio_shortcode($atts = []) {
        ...
        if (!in_array($mode, ['full', 'landing', 'demo', 'workflow', 'readiness', 'governance', 'room', 'packs', 'publication', 'outcomes', 'project-intake', 'scorecard', 'risk', 'scenario', 'handoff', 'packets', 'export', 'report', 'drawer', 'compact'], true)) {
            $mode = 'full';
        }
        ...
        $start_tab = $mode === 'workflow' ? 'workflow' : ($mode === 'readiness' ? 'readiness' : ($mode === 'governance' ? 'governance' : ($mode === 'room' ? 'room' : ($mode === 'packs' ? 'packs' : ($mode === 'publication' ? 'publication' : ($mode === 'outcomes' ? 'outcomes' : ($mode === 'project-intake' ? 'intake' : (in_array($mode, ['scorecard', 'risk', 'scenario', 'handoff', 'packets', 'export', 'report'], true) ? $mode : 'intake'))))))));
        ...
        wp_localize_script('scds-decision-studio', 'SCDSDecisionStudio', [
            ...
            'storageKey' => 'scds_saved_decision_packets_v1_14_0',
    ...
    private function public_landing_template() {
        return [
            ...
            'features' => [
                ...
                ['title'=>'Decision Packet workspace','description'=>'Use scds-decision-packet/1.7 to hold platform handoffs, evidence, assumptions, scenarios, experiments, calculations, entities, provenance, and audit records.'],
                ...
                ['title'=>'Decision Briefing and Publication Studio','description'=>'Generate governed internal, institutional, and public publications with Harvard citations, evidence anchors, visibility controls, redaction reports, and publication handoffs.'],['title'=>'Outcomes, Monitoring, and Reassessment','description'=>'Track commitments, targets, milestones, actual-versus-expected results, emerging risks, invalidated assumptions, reassessment triggers, amendments, retirement, and Decision Registry records.'],
    ...
    private function render_panel_readiness($mode) { ?>
        <section class="scds-panel" data-scds-panel="readiness">
            ...
            <div class="scds-note"><strong>v1.14.0:</strong> readiness scoring now surfaces section status, unresolved issues, required reviews, and export gates. It is a workflow quality screen, not approval or professional signoff.</div>
    ...
    private function render_panel_scenario($mode) { ?>
        <section class="scds-panel" data-scds-panel="scenario">
            ...
            <div class="scds-note"><strong>v1.14.0:</strong> scenario analysis is now an auditable studio rather than a fixed comparison table. Screening outputs remain conditional on the assumptions entered and should be routed to Workbench for probabilistic simulation, optimization, engineering, or domain forecasting.</div>
    ...
    private function render_panel_handoff($mode) { ?>
        <section class="scds-panel" data-scds-panel="handoff">
            ...
            <div class="scds-note"><strong>v1.14.0:</strong> handoff recommendations include tool IDs, reasons, priorities, shortcodes, and a payload summary that can be used to continue analysis in Workbench.</div>
    ...
    private function render_panel_export_center($mode) { ?>
        <section class="scds-panel" data-scds-panel="export">
            ...
            <div class="scds-note"><strong>v1.14.0:</strong> saved packets are working records for review and continuation. Reviewed and public exports are governed by the current approval gate; internal draft exports remain available for controlled working use.</div>
    ...
    private function render_panel_report($mode) { ?>
        <section class="scds-panel" data-scds-panel="report">
            ...
            <div class="scds-note"><strong>v1.14.0:</strong> the brief generator now includes readiness status, scenario comparison matrix, Workbench handoff details, audit appendix summary, and Markdown/HTML/JSON exports.</div>
            <div class="scds-actions">
    ...
    private function render_panel_audit($mode) { ?>
        <section class="scds-panel" data-scds-panel="audit">
            ...
            <div class="scds-note"><strong>v1.14.0:</strong> audit works with the readiness gate so unresolved evidence, source, calculation, finance, claim, and review issues can be surfaced before export.</div>
...
SCDS_OPENAI_MODEL=&lt;your-model&gt;</pre>';
    ...
    private function decision_packet_template() {
        return [
            'packet_version'=>self::VERSION,'decision_packet_schema'=>'scds-decision-packet/1.7','decision_pack_schema'=>self::DECISION_PACK_SCHEMA,'decision_pack_application_schema'=>self::DECISION_PACK_APPLICATION_SCHEMA,
            'publication_studio_schema'=>self::PUBLICATION_STUDIO_SCHEMA,
            'outcome_monitoring_schema'=>self::OUTCOME_MONITORING_SCHEMA,
            'reassessment_event_schema'=>self::REASSESSMENT_EVENT_SCHEMA,
            'decision_registry_schema'=>self::DECISION_REGISTRY_SCHEMA,
