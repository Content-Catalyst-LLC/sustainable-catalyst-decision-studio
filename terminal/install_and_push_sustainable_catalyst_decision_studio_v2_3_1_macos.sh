...
VERSION="2.3.1"
BASE_TAG="v2.3.0"
BASE_TARGET="590e109d9ba85cb9928fda7e44be203504410632"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scds-v231-install.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
BASE_ACTUAL="$(git -C "$REPO" rev-parse "${BASE_TAG}^{}" 2>/dev/null || true)"
[[ "$BASE_ACTUAL" == "$BASE_TARGET" ]] || fail "$BASE_TAG must remain at Energy Runtime Consumer commit $BASE_TARGET; found ${BASE_ACTUAL:-missing}"
git -C "$REPO" merge-base --is-ancestor "$BASE_TARGET" HEAD || fail "Current main does not contain the canonical v2.3.0 Energy Runtime Consumer"
[[ -f "$REPO/backend/app/energy_runtime_consumer.py" ]] || fail "Energy Runtime Consumer is missing before v2.3.1 install"

say "INSTALLING v2.3.1 RECONCILED RELEASE"
unzip -q "$ZIP" -d "$TMP/release"
SRC="$(find "$TMP/release" -mindepth 1 -maxdepth 3 -type f -name README.md -print -quit | xargs -I{} dirname "{}")"
[[ -n "$SRC" && -f "$SRC/backend/app/tradeoff_matrix.py" && -f "$SRC/backend/app/energy_runtime_consumer.py" ]] || fail "v2.3.1 source payload is incomplete"
...
say "VALIDATING v2.3.1 + PRESERVED ENERGY CONSUMER"
VENV="$TMP/venv"
"$PY" -m venv "$VENV"
...
cp "$REPO/deploy/contabo/upgrade_decision_studio_backend_v2_3_1_contabo.sh" "$DOWNLOADS/"
cp "$REPO/deploy/contabo/deploy_decision_studio_v2_3_1_from_macos.sh" "$DOWNLOADS/"
chmod +x "$DOWNLOADS/upgrade_decision_studio_backend_v2_3_1_contabo.sh" "$DOWNLOADS/deploy_decision_studio_v2_3_1_from_macos.sh"

say "COMMIT v2.3.1"
git add -A
if git diff --cached --quiet; then
  echo "Repository already matches v2.3.1."
else
  git commit -m "Decision Studio v2.3.1 — Criteria, Alternatives & Tradeoff Matrix"
fi
HEAD_SHA="$(git rev-parse HEAD)"

say "TAG v2.3.1"
if git rev-parse 'v2.3.1^{}' >/dev/null 2>&1; then
  EXISTING="$(git rev-parse 'v2.3.1^{}')"
  [[ "$EXISTING" == "$HEAD_SHA" ]] || fail "Existing v2.3.1 tag points to $EXISTING, not current HEAD $HEAD_SHA"
else
  git tag -a v2.3.1 -m "Decision Studio v2.3.1 — Criteria, Alternatives & Tradeoff Matrix"
fi
...
TAG_SHA="$(git rev-parse 'v2.3.1^{}')"
BASE_AFTER="$(git rev-parse 'v2.3.0^{}')"
printf 'HEAD:       %s\norigin/main:%s\nv2.3.1:     %s\nv2.3.0:     %s\n' "$HEAD_SHA" "$REMOTE_SHA" "$TAG_SHA" "$BASE_AFTER"
[[ "$HEAD_SHA" == "$REMOTE_SHA" && "$HEAD_SHA" == "$TAG_SHA" ]] || fail "v2.3.1 GitHub alignment failed"
[[ "$BASE_AFTER" == "$BASE_TARGET" ]] || fail "v2.3.0 Energy tag moved unexpectedly"
[[ -z "$(git status --porcelain)" ]] || fail "Working tree is not clean after v2.3.1"
echo "PASS: v2.3.1 is aligned and v2.3.0 Energy history is preserved."
...
echo "Deploy script: $DOWNLOADS/deploy_decision_studio_v2_3_1_from_macos.sh"
