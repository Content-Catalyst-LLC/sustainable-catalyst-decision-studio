...
VERSION="2.6.0"
BASE_TAG="v2.5.0"
BASE_TARGET="489c2c04d2fe8042762c5f796c0c9b71141bc7cc"
V240_TAG="v2.4.0"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scds-v260-install.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
[[ "$BASE_SHA" == "$BASE_TARGET" ]] || fail "$BASE_TAG must resolve to canonical v2.5.0 commit $BASE_TARGET; found ${BASE_SHA:-missing}"
...
[[ -f "$REPO/backend/app/energy_runtime_consumer.py" ]] || fail "Energy Runtime Consumer is missing before v2.6.0 install"
[[ -f "$REPO/backend/app/scenario_stress.py" ]] || fail "v2.5.0 Scenario/Stress module is missing before v2.6.0 install"

if git -C "$REPO" rev-parse 'v2.6.0^{}' >/dev/null 2>&1; then
  EXISTING="$(git -C "$REPO" rev-parse 'v2.6.0^{}')"
  fail "v2.6.0 tag already exists at $EXISTING. This installer never rewrites published tags."
fi

say "APPLYING v2.6.0 ADDITIVE OVERLAY"
unzip -q "$OVERLAY" -d "$TMP/overlay"
SRC="$(find "$TMP/overlay" -mindepth 1 -maxdepth 2 -type d -name sustainable-catalyst-decision-studio-v2.6.0-overlay -print -quit)"
[[ -n "$SRC" ]] || fail "Could not locate v2.6.0 overlay root"
for required in \
  backend/app/native_handoffs.py \
  backend/app/main.py \
  backend/tests/test_backend.py \
  data/analysis_handoff_contract_v2.6.0.json \
  data/computation_handoff_contract_v2.6.0.json \
  data/handoff_receipt_contract_v2.6.0.json \
  data/analysis_request_contract_v2.6.0.json \
  deploy/contabo/upgrade_decision_studio_backend_v2_6_0_contabo.sh \
  wordpress-plugin/sustainable-catalyst-decision-studio/sustainable-catalyst-decision-studio.php; do
  [[ -f "$SRC/$required" ]] || fail "v2.6.0 overlay missing $required"
done
...
say "VALIDATING LAB + WORKBENCH NATIVE HANDOFFS"
VENV="$TMP/venv"
"$PY" -m venv "$VENV"
...
bash -n deploy/contabo/upgrade_decision_studio_backend_v2_6_0_contabo.sh
bash -n deploy/contabo/deploy_decision_studio_v2_6_0_from_macos.sh
...
cp "$REPO/deploy/contabo/upgrade_decision_studio_backend_v2_6_0_contabo.sh" "$DOWNLOADS/"
cp "$REPO/deploy/contabo/deploy_decision_studio_v2_6_0_from_macos.sh" "$DOWNLOADS/"
chmod +x "$DOWNLOADS/upgrade_decision_studio_backend_v2_6_0_contabo.sh" "$DOWNLOADS/deploy_decision_studio_v2_6_0_from_macos.sh"

say "COMMIT v2.6.0"
git add -A
if git diff --cached --quiet; then
  fail "No v2.6.0 changes detected after overlay application."
fi
git commit -m "Decision Studio v2.6.0 — Lab + Workbench Native Handoffs"
HEAD_SHA="$(git rev-parse HEAD)"

say "TAG v2.6.0"
git tag -a v2.6.0 -m "Decision Studio v2.6.0 — Lab + Workbench Native Handoffs"
...
TAG_SHA="$(git rev-parse 'v2.6.0^{}')"
BASE_AFTER="$(git rev-parse 'v2.5.0^{}')"
V240_AFTER="$(git rev-parse 'v2.4.0^{}')"
V231_AFTER="$(git rev-parse 'v2.3.1^{}')"
ENERGY_AFTER="$(git rev-parse 'v2.3.0^{}')"
printf 'HEAD:       %s\norigin/main:%s\nv2.6.0:     %s\nv2.5.0:     %s\nv2.4.0:     %s\nv2.3.1:     %s\nv2.3.0:     %s\n' \
  "$HEAD_SHA" "$REMOTE_SHA" "$TAG_SHA" "$BASE_AFTER" "$V240_AFTER" "$V231_AFTER" "$ENERGY_AFTER"
[[ "$HEAD_SHA" == "$REMOTE_SHA" && "$HEAD_SHA" == "$TAG_SHA" ]] || fail "v2.6.0 GitHub alignment failed"
[[ "$BASE_AFTER" == "$BASE_TARGET" ]] || fail "v2.5.0 tag moved unexpectedly"
...
[[ -z "$(git status --porcelain)" ]] || fail "Working tree is not clean after v2.6.0"
echo "PASS: Decision Studio v2.6.0 is aligned; v2.5.0, v2.4.0, v2.3.1, and Energy v2.3.0 history remain intact."
...
echo "VPS deploy:    $DOWNLOADS/deploy_decision_studio_v2_6_0_from_macos.sh"
