...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scds-v240-install.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
[[ -n "$BASE_SHA" ]] || fail "$BASE_TAG is missing"
...
[[ -f "$REPO/backend/app/energy_runtime_consumer.py" ]] || fail "Energy Runtime Consumer is missing before v2.4.0 install"

say "INSTALLING v2.4.0 RELEASE"
unzip -q "$ZIP" -d "$TMP/release"
SRC="$(find "$TMP/release" -mindepth 1 -maxdepth 3 -type f -name README.md -print -quit | xargs -I{} dirname "{}")"
[[ -n "$SRC" && -f "$SRC/backend/app/uncertainty_confidence.py" && -f "$SRC/backend/app/tradeoff_matrix.py" && -f "$SRC/backend/app/energy_runtime_consumer.py" ]] || fail "v2.4.0 source payload is incomplete"
...
say "VALIDATING v2.4.0 + PRESERVED ENERGY CONSUMER"
VENV="$TMP/venv"
"$PY" -m venv "$VENV"
...
cp "$REPO/deploy/contabo/upgrade_decision_studio_backend_v2_4_0_contabo.sh" "$DOWNLOADS/"
cp "$REPO/deploy/contabo/deploy_decision_studio_v2_4_0_from_macos.sh" "$DOWNLOADS/"
chmod +x "$DOWNLOADS/upgrade_decision_studio_backend_v2_4_0_contabo.sh" "$DOWNLOADS/deploy_decision_studio_v2_4_0_from_macos.sh"

say "COMMIT v2.4.0"
git add -A
if git diff --cached --quiet; then
  echo "Repository already matches v2.4.0."
else
  git commit -m "Decision Studio v2.4.0 — Uncertainty, Sensitivity & Confidence"
fi
HEAD_SHA="$(git rev-parse HEAD)"

say "TAG v2.4.0"
if git rev-parse 'v2.4.0^{}' >/dev/null 2>&1; then
  EXISTING="$(git rev-parse 'v2.4.0^{}')"
  [[ "$EXISTING" == "$HEAD_SHA" ]] || fail "Existing v2.4.0 tag points to $EXISTING, not current HEAD $HEAD_SHA"
else
  git tag -a v2.4.0 -m "Decision Studio v2.4.0 — Uncertainty, Sensitivity & Confidence"
fi
...
TAG_SHA="$(git rev-parse 'v2.4.0^{}')"
BASE_AFTER="$(git rev-parse 'v2.3.1^{}')"
ENERGY_AFTER="$(git rev-parse 'v2.3.0^{}')"
printf 'HEAD:       %s\norigin/main:%s\nv2.4.0:     %s\nv2.3.1:     %s\nv2.3.0:     %s\n' "$HEAD_SHA" "$REMOTE_SHA" "$TAG_SHA" "$BASE_AFTER" "$ENERGY_AFTER"
[[ "$HEAD_SHA" == "$REMOTE_SHA" && "$HEAD_SHA" == "$TAG_SHA" ]] || fail "v2.4.0 GitHub alignment failed"
[[ "$ENERGY_AFTER" == "$ENERGY_TARGET" ]] || fail "v2.3.0 Energy tag moved unexpectedly"
[[ -z "$(git status --porcelain)" ]] || fail "Working tree is not clean after v2.4.0"
echo "PASS: Decision Studio v2.4.0 is aligned; v2.3.1 history and Energy v2.3.0 remain intact."
...
echo "VPS deploy:    $DOWNLOADS/deploy_decision_studio_v2_4_0_from_macos.sh"
