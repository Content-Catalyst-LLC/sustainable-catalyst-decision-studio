...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scds-v290-install.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
BASE_SHA="$(git -C "$REPO" rev-parse "${BASE_TAG}^{}" 2>/dev/null || true)"; [[ -n "$BASE_SHA" ]] || fail "v2.8.0 is not installed/tagged yet. Close v2.8.0 before applying v2.9.0."
[[ "$(git -C "$REPO" rev-parse HEAD)" == "$BASE_SHA" ]] || fail "v2.9.0 must start from the exact v2.8.0 tag. Current HEAD differs from $BASE_TAG."
V270_SHA="$(git -C "$REPO" rev-parse 'v2.7.0^{}' 2>/dev/null || true)"; [[ -n "$V270_SHA" ]] || fail "v2.7.0 historical tag missing"
...
[[ -f "$REPO/backend/app/dependency_graph.py" ]] || fail "v2.8 Decision Graph module missing"
...
if git -C "$REPO" rev-parse 'v2.9.0^{}' >/dev/null 2>&1; then fail "v2.9.0 tag already exists; installer never rewrites published tags."; fi
say "APPLYING v2.9.0 ADDITIVE OVERLAY"
unzip -q "$OVERLAY" -d "$TMP/overlay"
SRC="$(find "$TMP/overlay" -mindepth 1 -maxdepth 2 -type d -name 'sustainable-catalyst-decision-studio-v2.9.0-overlay' -print -quit)"; [[ -n "$SRC" ]] || fail "Could not locate v2.9.0 overlay root"
for required in backend/app/recommendation_review.py backend/app/main.py backend/tests/test_backend.py data/recommendation_candidate_contract_v2.9.0.json data/recommendation_challenge_contract_v2.9.0.json data/recommendation_review_contract_v2.9.0.json deploy/contabo/upgrade_decision_studio_backend_v2_9_0_contabo.sh wordpress-plugin/sustainable-catalyst-decision-studio/sustainable-catalyst-decision-studio.php; do [[ -f "$SRC/$required" ]] || fail "v2.9.0 overlay missing $required"; done
...
PY
"$PY" --version
say "VALIDATING RECOMMENDATIONS, REVIEW & CHALLENGE"
...
bash -n deploy/contabo/upgrade_decision_studio_backend_v2_9_0_contabo.sh; bash -n deploy/contabo/deploy_decision_studio_v2_9_0_from_macos.sh
...
cp deploy/contabo/upgrade_decision_studio_backend_v2_9_0_contabo.sh "$DOWNLOADS/"; cp deploy/contabo/deploy_decision_studio_v2_9_0_from_macos.sh "$DOWNLOADS/"; chmod +x "$DOWNLOADS/"*decision_studio*v2_9_0*.sh 2>/dev/null || true
say "COMMIT v2.9.0"; git add -A; git diff --cached --quiet && fail "No v2.9.0 changes detected"; git commit -m "Decision Studio v2.9.0 — Recommendations, Review & Challenge Layer"
HEAD_SHA="$(git rev-parse HEAD)"; say "TAG v2.9.0"; git tag -a v2.9.0 -m "Decision Studio v2.9.0 — Recommendations, Review & Challenge Layer"
if [[ "${SCDS_NO_PUSH:-0}" != "1" && -n "$REMOTE" ]]; then say "PUSHING GITHUB"; git push origin main; git push origin v2.9.0; fi
say "FINAL GITHUB VERIFICATION"; git fetch origin --prune --tags
REMOTE_SHA="$(git rev-parse origin/main)"; TAG_SHA="$(git rev-parse 'v2.9.0^{}')"; BASE_AFTER="$(git rev-parse 'v2.8.0^{}')"; V270_AFTER="$(git rev-parse 'v2.7.0^{}')"
printf 'HEAD:       %s\norigin/main:%s\nv2.9.0:     %s\nv2.8.0:     %s\nv2.7.0:     %s\n' "$HEAD_SHA" "$REMOTE_SHA" "$TAG_SHA" "$BASE_AFTER" "$V270_AFTER"
[[ "$HEAD_SHA" == "$REMOTE_SHA" && "$HEAD_SHA" == "$TAG_SHA" ]] || fail "v2.9.0 GitHub alignment failed"
[[ "$BASE_AFTER" == "$BASE_SHA" ]] || fail "v2.8.0 tag moved unexpectedly"; [[ "$V270_AFTER" == "$V270_SHA" ]] || fail "v2.7.0 tag moved unexpectedly"
...
echo "PASS: Decision Studio v2.9.0 aligned; v2.8.0 and all historical release tags remain intact."
echo "WordPress ZIP: $DOWNLOADS/sustainable-catalyst-decision-studio-plugin-v${VERSION}.zip"; echo "Backend ZIP: $DOWNLOADS/sustainable-catalyst-decision-studio-backend-v${VERSION}.zip"; echo "VPS deploy: $DOWNLOADS/deploy_decision_studio_v2_9_0_from_macos.sh"
