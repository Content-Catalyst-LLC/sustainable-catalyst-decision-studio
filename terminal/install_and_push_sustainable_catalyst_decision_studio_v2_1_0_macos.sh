...
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/scds-v210.XXXXXX")"
STASH_CREATED=0
...
if git diff --cached --quiet; then
  ...
  git commit -m "Decision Studio v${VERSION} — Unified Decision Object Model & Platform Context Foundation"
fi

if ! git rev-parse "v${VERSION}" >/dev/null 2>&1; then
  git tag -a "v${VERSION}" -m "Decision Studio v${VERSION} — Unified Decision Object Model & Platform Context Foundation"
else
