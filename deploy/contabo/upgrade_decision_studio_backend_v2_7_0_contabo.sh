...
VERSION="2.7.0"
ARCHIVE="${1:-/tmp/sustainable-catalyst-decision-studio-backend-v2.7.0.zip}"
...
TMP="$(mktemp -d /tmp/sc-decision-studio-v270.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
for required in \
  ...
  requirements.txt \
  Dockerfile; do
  [[ -f "$SRC/$required" ]] || fail "v2.7.0 backend payload missing $required"
done
grep -q 'APP_VERSION = "2.7.0"' "$SRC/app/main.py" || fail "payload is not Decision Studio v2.7.0"
grep -q 'scds-v2.7.0-site-intelligence-context-integration' "$SRC/app/main.py" || fail "v2.7.0 build fingerprint missing"
...
backup="$BACKUP_ROOT/decision-studio-before-v2.7.0-$stamp.tgz"
echo "=== PRE-FLIGHT ==="
...
s=re.sub(r'(?m)^(\s*image:\s*sustainable-catalyst-decision-studio:)[^\s#]+', r'\g<1>2.7.0', s)
s=re.sub(r'(?m)^(\s*SCDS_BUILD_FINGERPRINT:\s*).+$', r'\g<1>scds-v2.7.0-site-intelligence-context-integration', s)
s=re.sub(r'(?m)^(\s*SCDS_SOURCE_COMMIT:\s*).+$', r'\g<1>release-v2.7.0', s)
p.write_text(s)
PY
...
assert x.get('version') == '2.7.0'
assert x.get('build_fingerprint') == 'scds-v2.7.0-site-intelligence-context-integration'
assert x.get('source_commit') == 'release-v2.7.0'
...
assert x.get('version') == '2.7.0', x
assert x.get('service') == 'sustainable-catalyst-decision-studio', x
assert x.get('build_fingerprint') == 'scds-v2.7.0-site-intelligence-context-integration', x
...
print('PASS: internal health identifies Decision Studio backend v2.7.0 with v2.5 Scenario/Stress and Energy v2.3.0 preserved')
PY

echo "=== v2.7.0 SITE INTELLIGENCE CONTEXT CONTRACTS ==="
...
assert x.get('version')=='2.7.0',x
assert x.get('build_fingerprint')=='scds-v2.7.0-site-intelligence-context-integration',x
print('PASS: public Decision Studio API returns v2.7.0')
PY
  else
    echo "WARNING: public Caddy check did not answer; internal v2.7.0 verification passed."
  fi
fi

echo "=== FINAL CONTAINER ==="
docker ps --filter "name=$CONTAINER" --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}'
pass "Sustainable Catalyst Decision Studio backend v2.7.0 deployed and verified on port 8089"
echo "Backup: $backup"
