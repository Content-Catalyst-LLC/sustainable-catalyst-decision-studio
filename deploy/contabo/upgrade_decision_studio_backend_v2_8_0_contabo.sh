...
VERSION="2.8.0"
ARCHIVE="${1:-/tmp/sustainable-catalyst-decision-studio-backend-v2.8.0.zip}"
...
TMP="$(mktemp -d /tmp/sc-decision-studio-v280.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
for required in \
  ...
  app/site_intelligence_context.py \
  ...
  requirements.txt \
  Dockerfile; do
  [[ -f "$SRC/$required" ]] || fail "v2.8.0 backend payload missing $required"
done
grep -q 'APP_VERSION = "2.8.0"' "$SRC/app/main.py" || fail "payload is not Decision Studio v2.8.0"
grep -q 'scds-v2.8.0-decision-graph-dependency-mapping' "$SRC/app/main.py" || fail "v2.8.0 build fingerprint missing"
...
backup="$BACKUP_ROOT/decision-studio-before-v2.8.0-$stamp.tgz"
echo "=== PRE-FLIGHT ==="
...
s=re.sub(r'(?m)^(\s*image:\s*sustainable-catalyst-decision-studio:)[^\s#]+', r'\g<1>2.8.0', s)
s=re.sub(r'(?m)^(\s*SCDS_BUILD_FINGERPRINT:\s*).+$', r'\g<1>scds-v2.8.0-decision-graph-dependency-mapping', s)
s=re.sub(r'(?m)^(\s*SCDS_SOURCE_COMMIT:\s*).+$', r'\g<1>release-v2.8.0', s)
p.write_text(s)
PY
...
assert x.get('version') == '2.8.0'
assert x.get('build_fingerprint') == 'scds-v2.8.0-decision-graph-dependency-mapping'
assert x.get('source_commit') == 'release-v2.8.0'
...
assert r.get('site_intelligence_context_bundle_schema') == 'scds-site-intelligence-context-bundle/1.0'
assert r.get('site_intelligence_signal_snapshot_schema') == 'scds-site-intelligence-signal-snapshot/1.0'
assert r.get('site_intelligence_context_receipt_schema') == 'scds-site-intelligence-context-receipt/1.0'
assert r.get('decision_dependency_graph_schema') == 'scds-decision-dependency-graph/1.0'
assert r.get('dependency_diagnostics_schema') == 'scds-dependency-diagnostics/1.0'
assert r.get('change_impact_assessment_schema') == 'scds-change-impact-assessment/1.0'
PY
    then break; fi
  fi
  sleep 2
...
assert x.get('version') == '2.8.0', x
assert x.get('service') == 'sustainable-catalyst-decision-studio', x
assert x.get('build_fingerprint') == 'scds-v2.8.0-decision-graph-dependency-mapping', x
...
assert c.get('v2_7_0_site_intelligence_context_preserved') is True, c
assert c.get('decision_dependency_graphs') is True, c
assert c.get('dependency_mapping') is True, c
assert c.get('change_impact_tracing') is True, c
assert c.get('dependency_graph_implies_causality') is False, c
assert c.get('dependency_degree_implies_importance') is False, c
assert c.get('change_impact_implies_invalidation') is False, c
assert c.get('change_impact_changes_recommendation_automatically') is False, c
print('PASS: internal health identifies Decision Studio backend v2.8.0 with v2.5 Scenario/Stress and Energy v2.3.0 preserved')
PY

echo "=== v2.8.0 DECISION GRAPH & DEPENDENCY MAPPING ==="
graph_payload='{"decisionObject":{"schema":"scds-decision-object/1.0","decision_id":"D-V280-SMOKE","question":"Which resilience pathway should proceed?","evidence":[{"evidence_id":"e1","label":"Grid reliability evidence","source_product":"knowledge-library"},{"context_id":"ctx1","label":"Active outage signal","source_product":"site-intelligence","evidence_role":"contextual_evidence","scenario_refs":["s1"],"signal_fingerprint":"abc123"}],"assumptions":[{"assumption_id":"a1","label":"Demand growth","scenario_refs":["s1"]}],"models":[{"model_id":"m1","label":"Lifecycle cost model","evidence_refs":["e1"]}],"scenarios":[{"scenario_id":"s1","name":"Stress","score":71.5}],"criteria":[{"criterion_id":"c1","name":"Reliability"}],"alternatives":[{"alternative_id":"alt1","name":"Path A"}],"tradeoff_matrices":[{"matrix_id":"tm1","label":"Tradeoff matrix"}],"uncertainties":[{"uncertainty_id":"u1","label":"Demand uncertainty"}],"recommendations":[{"id":"r1","label":"Human-reviewed recommendation"}],"provenance":{"records":[]}},"explicitEdges":[{"from":"e1","to":"m1","relationship":"supports"}],"includeStructuralEdges":true}'
...
import json,sys
x=json.loads(sys.argv[1]); g=x['dependency_graph']; d=g['diagnostics']
assert x['ok'] is True,x
assert g['schema']=='scds-decision-dependency-graph/1.0',g
assert len(g['graph_fingerprint'])==64,g
assert d['node_count']>=8 and d['edge_count']>=5,d
assert all(e.get('causal_claim') is False and e.get('automatic_invalidation') is False for e in g['edges']),g
assert any(e['relationship']=='supports' and e['explicit'] for e in g['edges']),g
assert any(e['relationship']=='contextualizes' and e.get('metadata',{}).get('automatic_score_change') is False for e in g['edges']),g
print('PASS: dependency graph preserves explicit and structural relationships without causal claims')
PY
impact_payload="$(python3 - "$graph_build" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); g=x['dependency_graph']
a=next(n['node_id'] for n in g['nodes'] if n['node_type']=='assumption')
print(json.dumps({'graph':g,'changedNodeIds':[a]},separators=(',',':')))
PY
)"
impact_result="$(curl -fsS -H 'Content-Type: application/json' -X POST -d "$impact_payload" "http://127.0.0.1:${PORT}/decision-dependency-graph/impact")"
python3 - "$impact_result" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); i=x['change_impact']
assert x['ok'] is True,x
assert i['schema']=='scds-change-impact-assessment/1.0',i
assert i['review_queue'],i
assert i['automatic_invalidation'] is False and i['causal_claim'] is False and i['recommendation_changed'] is False,i
print('PASS: change impact creates a review queue without automatic invalidation or recommendation mutation')
PY
tampered_graph="$(python3 - "$graph_build" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); g=x['dependency_graph']; g['nodes'][0]['label']='tampered'
print(json.dumps({'graph':g},separators=(',',':')))
PY
)"
tampered_result="$(curl -fsS -H 'Content-Type: application/json' -X POST -d "$tampered_graph" "http://127.0.0.1:${PORT}/decision-dependency-graph/validate")"
python3 - "$tampered_result" <<'PY'
import json,sys
x=json.loads(sys.argv[1])
assert x['ok'] is False,x
assert any('fingerprint' in e for e in x['validation']['errors']),x
print('PASS: tampered dependency graph is rejected by fingerprint validation')
PY
attach_result="$(curl -fsS -H 'Content-Type: application/json' -X POST -d "$graph_payload" "http://127.0.0.1:${PORT}/decision-packet/dependency-graph")"
python3 - "$attach_result" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); o=x['decision_object']; p=x['decision_packet']
assert x['ok'] is True,x
assert o['decision_dependency_graphs'][-1]['schema']=='scds-decision-dependency-graph/1.0',o
assert o['provenance']['records'][-1]['action']=='decision_dependency_graph_attached',o
assert p['decision_dependency_graphs'][-1]['schema']=='scds-decision-dependency-graph/1.0',p
print('PASS: dependency graph attaches to Decision Object and Decision Packet with provenance')
PY

echo "=== v2.8.0 SITE INTELLIGENCE CONTEXT CONTRACTS ==="
site_contracts="$(curl -fsS "http://127.0.0.1:${PORT}/site-intelligence-context/contracts")"
python3 - "$site_contracts" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); c=x['contracts']
assert x['version']=='2.8.0',x
assert c['context_bundle_schema']=='scds-site-intelligence-context-bundle/1.0',c
assert c['signal_snapshot_schema']=='scds-site-intelligence-signal-snapshot/1.0',c
assert c['context_receipt_schema']=='scds-site-intelligence-context-receipt/1.0',c
print('PASS: Site Intelligence context contracts are registered')
PY

echo "=== SITE INTELLIGENCE CONTEXT BUNDLE ==="
site_payload='{"decisionObject":{"schema":"scds-decision-object/1.0","decision_id":"D-V270-SMOKE","scenarios":[{"scenario_id":"scenario:weather-disruption","score":73.2}],"provenance":{"records":[]}},"sourceVersion":"4.40.0","capturedAt":"2026-09-13T07:15:00Z","signals":[{"signal_id":"weather.active-alerts","category":"climate_environment","label":"Active weather alerts","value":3,"unit":"alerts","source_name":"NOAA / National Weather Service","source_url":"https://www.weather.gov/","geography":{"country":"USA","region":"Missouri"},"observed_at":"2026-09-13T07:00:00Z","updated_at":"2026-09-13T07:05:00Z","freshness_state":"live","methodology":{"method":"official alert feed"},"limitations":["Coverage depends on issuing authority and feed availability."],"scenario_refs":["scenario:weather-disruption"]}]}'
site_build="$(curl -fsS -H 'Content-Type: application/json' -X POST -d "$site_payload" "http://127.0.0.1:${PORT}/site-intelligence-context/build")"
python3 - "$site_build" <<'PY'
import hashlib,json,sys
x=json.loads(sys.argv[1]); b=x['context_bundle']; s=b['snapshots'][0]
raw=json.dumps(s['raw_payload'],sort_keys=True,separators=(',',':'),ensure_ascii=False).encode()
assert x['ok'] is True and x['receipt']['accepted'] is True,x
assert b['schema']=='scds-site-intelligence-context-bundle/1.0',b
assert s['source']['name']=='NOAA / National Weather Service',s
assert s['geography']['region']=='Missouri',s
assert s['observed_at']=='2026-09-13T07:00:00Z',s
assert s['freshness_state']=='live',s
assert s['signal_fingerprint']==hashlib.sha256(raw).hexdigest(),s
assert x['receipt']['truth_verified'] is False,x
assert x['receipt']['causality_inferred'] is False,x
assert x['receipt']['recommendation_generated'] is False,x
print('PASS: Site Intelligence context preserves source/geography/time/freshness with bounded receipt semantics')
PY

echo "=== SITE CONTEXT DECISION OBJECT ATTACHMENT ==="
site_attach="$(curl -fsS -H 'Content-Type: application/json' -X POST -d "$site_payload" "http://127.0.0.1:${PORT}/decision-object/site-intelligence-context")"
python3 - "$site_attach" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); o=x['decision_object']
assert x['ok'] is True,x
assert o['scenarios'][0]['score']==73.2,o
assert o['real_world_context'][-1]['evidence_role']=='contextual_evidence',o
assert o['real_world_context'][-1]['causal_claim'] is False,o
links=[z for z in o['links'] if z.get('relationship')=='explicit_site_context_for_scenario']
assert links and links[-1]['automatic_score_change'] is False,o
assert o['provenance']['records'][-1]['action']=='site_intelligence_context_attached',o
print('PASS: Site Intelligence context attaches without changing scenario scores or inferring causality')
PY

echo "=== SITE CONTEXT TAMPER REJECTION ==="
tampered_site="$(python3 - "$site_build" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); b=x['context_bundle']; b['snapshots'][0]['raw_payload']['value']=999
print(json.dumps({'contextBundle':b},separators=(',',':')))
PY
)"
tampered_result="$(curl -fsS -H 'Content-Type: application/json' -X POST -d "$tampered_site" "http://127.0.0.1:${PORT}/site-intelligence-context/validate")"
python3 - "$tampered_result" <<'PY'
import json,sys
x=json.loads(sys.argv[1])
assert x['ok'] is False,x
...
assert any('fingerprint' in e for e in x['validation']['errors']),x
print('PASS: tampered Site Intelligence source payload is rejected')
PY
...
assert x.get('version')=='2.8.0',x
assert x.get('build_fingerprint')=='scds-v2.8.0-decision-graph-dependency-mapping',x
print('PASS: public Decision Studio API returns v2.8.0')
PY
  else
    echo "WARNING: public Caddy check did not answer; internal v2.8.0 verification passed."
  fi
fi

echo "=== FINAL CONTAINER ==="
docker ps --filter "name=$CONTAINER" --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}'
pass "Sustainable Catalyst Decision Studio backend v2.8.0 deployed and verified on port 8089"
echo "Backup: $backup"
