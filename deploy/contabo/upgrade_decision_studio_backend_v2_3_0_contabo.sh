...
VERSION="2.3.0"
TARGET_KEY="decision-studio"
PRODUCT="Decision Studio"
ARCHIVE="${1:-/tmp/sustainable-catalyst-decision-studio-backend-v2.3.0.zip}"
ROOT="${SC_TARGET_ROOT:-/opt/sustainable-catalyst/decision-studio}"
LIVE_BACKEND="$ROOT/backend"
COMPOSE="${SC_TARGET_COMPOSE:-$ROOT/compose.yml}"
SERVICE="${SC_TARGET_SERVICE:-decision-studio}"
CONTAINER="${SC_TARGET_CONTAINER:-sc-decision-studio}"
BACKUP_ROOT="/opt/sustainable-catalyst/backups"
TMP="$(mktemp -d /tmp/sc-energy-consumer-decision-studio.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
tar -C "$ROOT" -czf "$BACKUP_ROOT/decision-studio-backend-before-v$VERSION-$stamp.tgz" backend
cp -a "$COMPOSE" "$BACKUP_ROOT/decision-studio-compose-before-v$VERSION-$stamp.yml"
...
f=framework()
assert f['consumer_version']=='2.3.0', f
assert f['target_key']=='decision-studio', f
...
print('PASS: Decision Studio v2.3.0 Energy Systems contract intake is active')
